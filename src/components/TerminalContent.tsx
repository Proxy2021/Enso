  ...
  return (
    <div className="mb-1.5 pl-3">
      <button
        ...
        className="flex items-center gap-1.5 text-[11px] text-purple-400/80 hover:text-purple-300 transition-all duration-150"
      >
        {isActive ? (
...
export function ActivityIndicator({ label }: { label?: string }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2 pl-3 text-xs">
      ...
      <span className="text-gray-600 font-mono tabular-nums">{formatElapsed(elapsed)}</span>
    </div>
  );
}
...
export function PromptSuggestion({ suggestion, onSelect }: { suggestion: string; onSelect: (text: string) => void }) {
  return (
    <button
      ...
      className="text-left px-2 py-1 mt-0.5 rounded border border-gray-700/50 bg-gray-900/40 hover:bg-gray-800/60 hover:border-gray-600 transition-all duration-150 cursor-pointer group"
    >
      <span className="text-gray-500 text-[11px] italic group-hover:text-gray-300 transition-all duration-150">{suggestion}</span>
    </button>
  );
}
