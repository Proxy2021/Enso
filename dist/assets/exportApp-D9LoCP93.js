...
`;function y(e){const{escapedData:r,escapedJsx:t,wsUrl:o,cardId:l,serverUrl:m,serverToken:s,toolMeta:n}=e;return`
    // ── Config ──
    const WS_URL = ${JSON.stringify(o)};
    const CARD_ID = ${JSON.stringify(l)};
    const SERVER_URL = ${JSON.stringify(m)};
    const SERVER_TOKEN = ${JSON.stringify(s)};
    const TOOL_META = ${n};
    const INITIAL_DATA = ${r};
    const INITIAL_JSX = ${t};
...
`}function v(e,r,t){var i,d;const o=e.appGeneratedUI??e.generatedUI??"",l=e.appData??e.data??{},s=`Enso — ${(((i=e.appCardMode)==null?void 0:i.toolFamily)??((d=e.cardMode)==null?void 0:d.toolFamily)??"app").replace(/_/g," ")}`,n=JSON.stringify(o),c=JSON.stringify(l);let a;if(r==="live"&&t){const p=t.serverUrl.startsWith("https")?"wss:":"ws:",g=new URL(t.serverUrl),b=`${p}//${g.host}/ws${t.token?`?token=${encodeURIComponent(t.token)}`:""}`,f=e.toolMeta?JSON.stringify(e.toolMeta):"null";a=y({escapedData:c,escapedJsx:n,wsUrl:b,cardId:e.id,serverUrl:t.serverUrl,serverToken:t.token,toolMeta:f})}else a=h(c,n);return`<!DOCTYPE html>
...
</html>`}function R(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function E(e,r="offline",t){var i,d;const o=v(e,r,t),l=((i=e.appCardMode)==null?void 0:i.toolFamily)??((d=e.cardMode)==null?void 0:d.toolFamily)??"app",s=`${r==="live"?"enso-live":"enso"}-${l.replace(/_/g,"-")}-${Date.now()}.html`,n=new Blob([o],{type:"text/html"}),c=URL.createObjectURL(n),a=document.createElement("a");a.href=c,a.download=s,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(c)}export{E as exportCardAsHtml};
