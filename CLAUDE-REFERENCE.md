# Enso — CLAUDE-REFERENCE.md

Detailed reference material for building Enso apps, API surfaces, template rules, and best practices. For the concise overview, see [CLAUDE.md](CLAUDE.md).
...
## ExecutorContext — Full API Reference
...
## EnsoUI Component Library — Full Reference
...
The `/tool enso` command opens a dedicated in-app tool console card.
...
4. It writes files directly to `server/apps/<family>/`
5. Restart server to load the new app

**Method 3: Manual (for full control)**
1. Create `server/apps/<family>/` directory
...
Click bookmark icon in Apps menu
    ↓
Copied to server/apps/<family>/     (shipped app)
    ↓
...
```
1. server/src/<family>-tools.ts       ← Tool functions
...
**File:** `server/src/<family>-tools.ts`
...
**File:** `server/src/app-catalog.ts`
...
**File:** `server/src/native-tools/templates/<family>.ts`
...
**File:** `server/src/native-tools/registry.ts`
