...
function buildEvolutionPlanningPrompt(
  ...
  const lines = [
    ...
    `The team picks the TOP 1-2 highest-impact, medium-or-lower-effort enhancements from the discussion and ACTUALLY IMPLEMENTS them in the Enso codebase.`,
    ...
    `- **Instructions:** Read \`.evolution-synthesis.md\` and all persona reports. Simulate a product team roundtable discussion where EACH persona's voice is represented. Debate priorities and trade-offs. End with a FINAL DECISION: the top 1-2 enhancements to implement THIS sprint (pick highest-impact, lowest-effort). For each chosen enhancement, write a clear SPEC: what to change, which files, expected behavior before/after. Write \`.evolution-discussion.md\`.`,
    ``,
    `## Phase 3: Engineering Implementation`,
    ``,
    `### Design Solution`,
    ...
    ...validationPersonas.map((p, i) => [
      ...
      `  1. Read \`.evolution-implementation.md\` to know what changed`,
      `  2. Read the original report \`.evolution-persona-${p.id}.md\` for baseline scores`,
      `  3. Write a NEW Puppeteer test script \`retest-persona-${p.id}.mjs\` that tests the SPECIFIC areas that were enhanced`,
      `  4. Execute the script: \`node retest-persona-${p.id}.mjs\``,
      `  5. Compare before/after experience from ${p.name}'s perspective`,
