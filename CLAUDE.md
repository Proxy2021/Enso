...
│   └── ...                   # Other card renderers (chat, terminal, user-bubble)
├── components/
...
└── src/
    ...
    ├── server.ts             # Express + WS server, handles card.action routing
...
└── types.ts                  # Protocol: ClientMessage, ServerMessage, ToolRouting
```
...
- **Client → Server**: `ClientMessage` with types `chat.send`, `chat.history`, `ui_action`, `card.action`
...
- Components cached by data shape hash (DJB2) — identical structures reuse components
...
**Implemented**: Card-based chat UI, WebSocket messaging, text streaming (delta/final), dynamic React component generation, Gemini UI generation, component caching, media upload, OpenClaw plugin integration, interactive card actions (onAction → server mutation → UI regeneration → in-place card update), card interaction context with action history.

**Not yet implemented**: Persistent chat history, agent-in-the-loop card actions (currently mechanical), user authentication, multi-user sessions, rate limiting, request logging.
