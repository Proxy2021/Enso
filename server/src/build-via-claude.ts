...
// ─── Argument Graph Bespoke Build (Variant B) ────────────────────────────────

interface ArgumentGraphBuildParams {
  entityId: string;
  graphJson: string;       // serialized ArgumentGraph to embed + visualize
  bookTitle: string;
  bookAuthor: string;
  cardId: string;          // book detail card that initiated this
  client: ConnectedClient;
  account: ResolvedEnsoAccount;
}

/**
 * Claude-Code-generated bespoke argument graph visualization. Takes a
 * pre-generated structured graph (nodes + edges + thesis + crux) and asks
 * Claude Code to design a custom interactive visualization tailored to this
 * specific book's argument shape. Output → `.argument-graph-ui.jsx` → returned
 * as a JSX string for delivery as `argumentGraphBespokeUI` on the book card.
 */
export async function handleArgumentGraphBuild(params: ArgumentGraphBuildParams): Promise<string | null> {
  const { entityId, graphJson, bookTitle, bookAuthor, cardId, client } = params;
  const runId = randomUUID();
  const outputFile = join(PROJECT_ROOT, ".argument-graph-ui.jsx");

  logAction({ ts: Date.now(), type: "build", category: "argument-graph-build", message: `Bespoke arg graph build start: "${bookTitle}"`, cardId });

  try { if (existsSync(outputFile)) unlinkSync(outputFile); } catch { /* ignore */ }

  const prompt = buildArgumentGraphPrompt(bookTitle, bookAuthor, graphJson);
  ...
  try {
    ...
    logError("argument-graph-build", "Claude Code session error", err, { cardId, entityId });
    return null;
  }

  if (!existsSync(outputFile)) {
    logError("argument-graph-build", "No output file after Claude Code session", undefined, { cardId, entityId });
    return null;
  }
  ...
  try {
    ...
    logError("argument-graph-build", "Failed to read output file", err, { cardId, entityId });
    return null;
  }

  if (!templateJSX || templateJSX.length < 100) {
    logError("argument-graph-build", "Output file too small or empty", undefined, { cardId, entityId });
    return null;
  }

  // Compile-check + auto-fix
  try {
    const { transform } = await import("sucrase");
    transform(templateJSX, { transforms: ["jsx"], jsxRuntime: "classic" });
    logAction({ ts: Date.now(), type: "build", category: "argument-graph-build", message: "Compile check OK", cardId });
  } catch (compileErr) {
    logAction({ ts: Date.now(), type: "build", category: "argument-graph-build", message: "Compile error, attempting auto-fix", cardId });
    if (sessionId) {
      writeFileSync(outputFile, templateJSX);
      const fixPrompt = [
        "The .argument-graph-ui.jsx you just created has a compile error:",
        "```",
        String(compileErr),
        "```",
        "",
        "Fix the file. Common issues: unclosed JSX tags, missing parentheses, using Tooltip (Recharts) where you meant EnsoUI.Tooltip. No imports allowed.",
      ].join("\n");
      try {
        await runClaudeCode({
          prompt: fixPrompt,
        ...
        if (existsSync(outputFile)) {
          ...
          const { transform } = await import("sucrase");
          transform(templateJSX, { transforms: ["jsx"], jsxRuntime: "classic" });
          logFix({ description: "Argument graph compile error", error: String(compileErr), resolution: "Auto-fixed via Claude Code", category: "argument-graph-build" });
        } else {
          return null;
        }
      } catch (fixErr) {
        logError("argument-graph-build", "Auto-fix failed", fixErr, { cardId });
        try { unlinkSync(outputFile); } catch { /* ignore */ }
        return null;
      }
    } else {
  ...
  logAction({ ts: Date.now(), type: "build", category: "argument-graph-build", message: `Bespoke arg graph ready for "${bookTitle}" (${templateJSX.length} chars)`, cardId });
  return templateJSX;
}

function buildArgumentGraphPrompt(bookTitle: string, bookAuthor: string, graphJson: string): string {
  return `You are designing a bespoke interactive visualization of the ARGUMENT STRUCTURE of "${bookTitle}" by ${bookAuthor}. The structured graph has already been extracted — your job is to render it as the best possible interactive experience for this specific book's argument shape.

## Input: Argument Graph JSON
The argument graph is provided below. Nodes have: id, type (thesis/claim/assumption/evidence/counter/conclusion), label, description, optional chapter. Edges have: from, to, relation (supports/requires/exemplifies/contradicts/weakens/concludes), optional note. One node is flagged as the crux (cruxNodeId) — the single load-bearing claim.
...
Write ONE file at \`.argument-graph-ui.jsx\` containing a single self-contained JSX component. DO NOT do any research — the graph is already complete. Your only job is the visualization.

Design an interactive experience that:
1. **Exposes the argument skeleton** — the reader should see at a glance which claims are load-bearing and which are peripheral.
2. **Highlights the crux node visibly** — larger, ringed, distinctly colored.
3. **Shows relation types clearly** — supports/requires/contradicts should look different (color, arrow style, dash pattern).
4. **Invites exploration** — clicking a node opens a detail panel with its description and all connected edges. Clicking the crux surfaces a "what happens if this falls" breakdown of downstream nodes.
5. **Surfaces assumptions and counters** — these are the highest-value nodes (what readers miss). Make them easy to find.

## Design variations — pick ONE that fits this book's argument shape

- **Layered pyramid**: thesis at top, claims beneath, evidence + assumptions at base. Good for deductive books.
- **Radial**: crux at center, claims radiating out, counters on the periphery. Good for books built around a single central claim.
- **Dependency chain**: left-to-right, showing "A requires B requires C." Good for sequential arguments.
- **Pros-vs-cons columns**: supporting evidence on one side, counters on the other, claims bridging them. Good for contested books.
- **Galaxy**: claim clusters grouped by theme, with inter-cluster links. Good for sprawling books.

Pick ONE based on this book's shape. Don't default to layered — if counters loom large, use pros-vs-cons; if everything hangs on one claim, use radial.

## Hard rules

- Sandbox environment: NO imports. React hooks, Recharts, Lucide icons, and EnsoUI components are in scope.
- Render the graph with inline SVG — d3/dagre/reactflow are NOT available. Lay nodes out manually using arithmetic (x/y coordinates).
- EMBED the graph JSON inside the component as a \`const\` so it renders with no external data.
- State via React.useState at the top level only.
- Use EnsoUI.Tooltip (NOT Tooltip — that's Recharts).
- No fetch, no window/document globals beyond standard React-safe ones.
- Include a short legend explaining node types + relation types.
- Dark theme; background should match #0f172a-ish.

## Output

Write ONLY the JSX component to \`.argument-graph-ui.jsx\`. No prose, no markdown fences around the file. A single export-less component function that the sandbox will render with \`data\` passed in as a prop (ignore it — embed the graph directly).`;
}
