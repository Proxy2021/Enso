...
describe("saveApp + loadApps round-trip", () => {
  it("saves and loads an app correctly", () => {
    ...
    const appDir = path.join(tmpBase, "apps", "workout_planner");
  ...
  it("skips corrupt app directories", () => {
    ...
    const corruptDir = path.join(tmpBase, "apps", "corrupt_app");
  ...
  it("skips apps with missing required manifest fields", () => {
    const appDir = path.join(tmpBase, "apps", "bad_manifest");
...
describe("loadAndRegisterApps", () => {
  it("loads and registers all saved apps (user + codebase)", () => {
    ...
    // count includes user apps + any codebase apps from server/apps/
  ...
  it("loads codebase apps even when no user apps exist", () => {
    // tmpBase has no user apps, but codebase apps from server/apps/ are loaded
